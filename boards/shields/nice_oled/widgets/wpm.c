...
static void draw_needle(lv_obj_t *canvas, const struct status_state *state) {
    ...
    float max = CONFIG_NICE_OLED_WPM_FIXED_RANGE_MAX;
#else
    float max = 0;
    for (int i = 0; i < 10; i++) {
        if (state->wpm[i] > max) {
...
static void draw_label(lv_obj_t *canvas, const struct status_state *state) {
    ...
    lv_canvas_draw_text(canvas, 0, 101, 25, &label_left_dsc, "WPM");
    ...
    lv_canvas_draw_text(canvas, 26, 101, 42, &label_dsc_wpm, wpm_text);
}
